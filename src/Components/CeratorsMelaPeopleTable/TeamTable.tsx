...
const TeamTable = ({ search }) => {
    ...
    const fetchTeams = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/people/team/list/?limit=${limit}&offset=${offset}${searchQuery}`,
          {
            headers: {
  ...
  const totalPages = teamsData
    ? Math.ceil(teamsData.count / recordsPerPage)
    : 0;

  const currentData = teamsData?.results?.length > 0 ? teamsData.results : [];
  ...
  // console.log(teamData.results.length, "TeamData");
  // useEffect(() => {
  ...
  // const totalPages = Math.ceil(teamData.results.length / recordsPerPage);

  // const teamsData = teamsData.slice(
